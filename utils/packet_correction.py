...
def act(ugly_duckling, white_swan, sync_header, sync_footer, second_sync_header, second_sync_footer,
    ...
    """
    执行时间戳修改
    :param drift: 使用时间平移/漂移
    :param delta: 累加时间差量，时间戳格式
    :param moment: 开始时间，即自定义时间
    :param second_sync_footer: 二级标识符尾，十六进制字符串形式
    :param second_sync_header: 二级标识符头，十六进制字符串形式
    :param sync_footer: 标识符尾，十六进制字符串形式
    :param ugly_duckling: 修改前十六进制文件
    :param white_swan: 修改后十六进制文件
    :param sync_header: 标识符头，十六进制字符串形式
    :return: 修改成功返回 True
    """
    ...
    restore_hex.put_back(parent_sequence=ugly_duckling, sub_sequence=white_swan, target_list=target_length[0],
...
if __name__ == "__main__":
    ...
    # 查看源文件是否正确写入
    view_hex.look_over(file_name=wechat_file)
    ...
    # 查看提取的新文件
    view_hex.look_over(file_name=new_file)
    ...
    restore_hex.put_back(parent_sequence=wechat_file, sub_sequence=new_file, target_list=targeting_length[0],
